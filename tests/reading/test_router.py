...
def test_epub_pairing_requires_confirmation_and_preserves_the_derived_material(
    client: TestClient,
) -> None:
    ...
    body = created.json()
    derived_id = body["material"]["material_id"]
    raw = client.get(f"/api/v1/reading/materials/{derived_id}/raw")
    assert raw.status_code == 200
    assert b"dt-bilingual-zh" in raw.content
    ...
    assert client.get("/api/v1/reading/epub-pairings").json() == []
    assert client.get(f"/api/v1/reading/materials/{derived_id}/raw").status_code == 200
