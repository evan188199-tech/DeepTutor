"""Persistence tests for KB github_sources / web_sources metadata."""
...
def test_update_github_source_state_persists(tmp_path: Path) -> None:
    manager, metadata_file, kb = _make_manager_with_kb(tmp_path)
    info = manager.add_github_source(kb, "owner/repo")
    manager.update_github_source_state(kb_name=kb, source_id=info["id"],
                                       last_synced_sha="abc", last_sync_status="success")
    on_disk = json.loads(metadata_file.read_text(encoding="utf-8"))
...
def test_add_web_source_persists(tmp_path: Path) -> None:
    ...
    on_disk = json.loads(metadata_file.read_text(encoding="utf-8"))
    assert len(on_disk["web_sources"]) == 1

def test_add_web_source_idempotent(tmp_path: Path) -> None:
    manager, _, kb = _make_manager_with_kb(tmp_path)
    info1 = manager.add_web_source(kb, "https://example.com/docs/")
    info2 = manager.add_web_source(kb, "https://example.com/docs/")
    assert info1["id"] == info2["id"]
...
def test_update_web_source_state_persists(tmp_path: Path) -> None:
    manager, metadata_file, kb = _make_manager_with_kb(tmp_path)
    info = manager.add_web_source(kb, "https://example.com/docs/")
    manager.update_web_source_state(kb_name=kb, source_id=info["id"],
                                    page_count=5, last_sync_status="success")
    on_disk = json.loads(metadata_file.read_text(encoding="utf-8"))
    assert on_disk["web_sources"][0]["page_count"] == 5
