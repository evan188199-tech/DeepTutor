...
@pytest.mark.asyncio
async def test_mcp_pat_is_scoped_revocable_and_marks_writes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert tools["search_media"].annotations and tools["search_media"].annotations.readOnlyHint is True
    assert tools["create_playlist"].annotations and tools["create_playlist"].annotations.readOnlyHint is False
    assert tools["remove_playlist_items"].annotations and tools["remove_playlist_items"].annotations.destructiveHint is True
