...
def test_ingest_handles_deletions(tmp_path: Path) -> None:
    ...
    store.ingest(
        SyncBatch(
            device_id="dev1",
            objects=[
                MarginNoteObject(object_id="extra1", object_type=NOTE, device_id="dev1"),
                MarginNoteObject(object_id="extra2", object_type=NOTE, device_id="dev1"),
            ],
        )
    )
    result = store.ingest(SyncBatch(device_id="dev1", deleted_ids=["note1"]))
    assert result.deleted == 1
    assert store.count(device_id="dev1") == 4
