...
def test_vocabulary_review_and_export_router_contracts(
    ...
    package_path = tmp_path / "vocabulary.apkg"
    package_path.write_bytes(b"fake-apkg")
    ...
    class FakeService:
        ...
        def export_vocabulary_apkg(self):
            return package_path
    ...
    packaged = client.get("/api/v1/immersive-reading/vocabulary/export/apkg")
    ...
    assert packaged.content == b"fake-apkg"
    assert packaged.headers["content-type"].startswith("application/octet-stream")


def test_vocabulary_difficulty_router_contracts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import deeptutor.api.routers.immersive_reading as router_module
    ...
    preview = {
        "available": True,
        "reason": "",
        "words": [{"word": "ephemeral", "band": "low"}],
        "distribution": {"low": 1},
    }

    class FakeService:
        def get_section(self, document_id: str, section_id: str):
            assert (document_id, section_id) == ("document-1", "section-1")
            return {"content": "The ephemeral light."}

        def analyze_vocabulary_difficulty(self, content: str):
            assert content == "The ephemeral light."
            return preview

    class FakePairingService:
        def get_bilingual_section(self, pairing_id: str, chapter_id: str):
            assert (pairing_id, chapter_id) == ("pairing-1", "chapter-1")
            return {"groups": [{"en": ["The ephemeral light."]}]}
    ...
    document = client.get(
        "/api/v1/immersive-reading/documents/document-1/sections/section-1/vocabulary-difficulty"
    )
    bilingual = client.get(
        "/api/v1/immersive-reading/bilingual/pairing-1/section/chapter-1/vocabulary-difficulty"
    )

    assert document.status_code == 200, document.text
    assert document.json() == preview
    assert bilingual.status_code == 200, bilingual.text
    assert bilingual.json() == preview
...
def test_generated_cards_use_bilingual_context_and_stable_ids() -> None:
    ...
    assert [card.id for card in generated.cards] == [
        "id-bright:cloze",
        "id-bright:choice",
    ]
...
def test_review_queue_prefers_due_age_then_frequent_words() -> None:
    due_old = _vocab_entry("old", review={"due_at": 100, "interval_index": 2, "review_count": 3})
...
def test_review_grading_uses_lightweight_spaced_repetition() -> None:
    ...
    entries, correct_twice = grade_review(entries, entry.id, correct=True, now=now + 10)
...
def test_apkg_export_contains_standard_collection_and_both_cards() -> None:
    ...
    note = connection.execute("SELECT guid, mid, flds FROM notes").fetchone()
...
def test_chapter_difficulty_uses_frequency_and_oxford_fields(
    tmp_path: Path,
) -> None:
    ...
    source.write_text(
        "\n".join(
            [
                ...
                'ephemeral,"ɪfemərəl","""adj. lasting a short time""",'
                '"""adj. 短暂的""",adj,,21000,0',
            ]
        ),
        encoding="utf-8",
...
def test_old_ecdict_schema_reports_unavailable_but_still_looks_up(
    tmp_path: Path,
) -> None:
    ...
    connection.executescript(
        ...
        INSERT INTO entries VALUES (
          'harbour', 'harbour', '/hɑrbər/', 'n. port', 'n. 港口', 'n', ''
        );
        """
    )
    connection.commit()
