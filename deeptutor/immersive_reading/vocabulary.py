...
def _distractors(entries: Iterable[VocabEntry], answer: str, limit: int = 3) -> list[str]:
    candidates = [entry.word for entry in entries if entry.word.casefold() != answer.casefold()]
...
def ensure_cards(entry: VocabEntry, all_entries: Iterable[VocabEntry]) -> VocabEntry:
    ...
    now = time.time()
    updated_at = entry.cards[-1].updated_at if entry.cards else now
    cards = [
        VocabularyCard(
            id=f"{entry.id}:cloze",
            card_type="cloze",
    ...
    *,
    ...
    for entry in entries:
        ...
        if correct:
            ...
            state.interval_index = min(len(REVIEW_INTERVALS_DAYS) - 1, state.interval_index + 1)
...
def _collection_anki2(
    entries: list[VocabEntry], deck_name: str, model_id: int, deck_id: int
) -> bytes:
    now = int(time.time())
    field_names = [
        "Word",
    ...
    templates = [
        {
            ...
            "afmt": (
                "{{FrontSide}}<hr id=answer><b>{{Word}}</b> {{Phonetic}}"
                "<br>{{English Definition}}<br>{{Chinese Definition}}"
        ...
        },
        {
            ...
            "ord": 1,
            "qfmt": (
                "{{Cloze Sentence}}<br>{{Choices}}<br><small>Which word fits the blank?</small>"
            ),
            "afmt": (
                "{{FrontSide}}<hr id=answer><b>{{Word}}</b> {{Phonetic}}"
                "<br>{{English Definition}}<br>{{Chinese Definition}}"
    ...
    model = {
        ...
        "css": (
            ".card{font-family:system-ui,sans-serif;font-size:20px;"
            "line-height:1.5;color:#222}.zh{color:#555}.cloze{font-weight:700}"
            "small{color:#777}"
        ),
    ...
    connection.executescript(
        ...
        CREATE TABLE cards (
          ...
          usn integer not null, ord integer not null, type integer not null,
          queue integer not null, due integer not null, ivl integer not null,
          factor integer not null, reps integer not null, lapses integer not null,
          left integer not null, odue integer not null, odid integer not null,
          flags integer not null, data text not null
        );
        CREATE TABLE revlog (
    ...
    for index, entry in enumerate(entries, start=1):
        choices = entry.cards[1].choices if len(entry.cards) > 1 else []
        fields = [
            entry.word,
            entry.phonetic,
            ...
            entry.chinese,
            "<ul>"
            + "".join(f"<li>{_field(choice)}</li>" for choice in choices if choice != entry.word)
            + "</ul>",
            f"{entry.document_title} \u00b7 {entry.section_title}"
            if entry.document_title or entry.section_title
            else "Immersive Reading",
        ]
...
def vocabulary_apkg(
    entries: Iterable[VocabEntry], deck_name: str = "DeepTutor Vocabulary"
) -> bytes:
    ...
    with zipfile.ZipFile(package, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr(
            "collection.anki2",
            _collection_anki2(materialized, deck_name, model_id, deck_id),
        )
...
def _frequency_band(
    rank: int, oxford: int
) -> Literal["core", "common", "advanced", "low", "unknown"]:
    ...
    *,
    ...
    try:
        ...
        reason = (
            ""
            if available
            else "ECDICT frequency fields are unavailable; "
            "reimport an ECDICT CSV with frq and oxford columns."
        )
