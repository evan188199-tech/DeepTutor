...
ABBREVIATIONS = {
    "mr", "mrs", "ms", "dr", "prof", "sr", "jr", "st", "co", "inc", "ltd", "corp",
    "vs", "v", "e.g", "i.e", "etc", "cf", "vol", "vols", "no", "nos", "sec", "fig",
    "figs", "p", "pp", "ch", "ed", "eds", "trans", "app", "dept", "univ", "approx",
    "ibid", "u.s", "u.k", "u.n", "d.c", "a.m", "p.m",
}
...
def feature_set(text: str) -> dict:
    ...
    stop = {
        ...
        "have", "has", "had", "not", "but", "they", "their", "them", "what", "when",
        "where", "than", "then", "into", "onto", "about", "over", "under", "after",
        "before", "also", "only", "more", "most", "such", "some", "any", "all", "can",
        "could", "would", "should", "may", "might", "will", "shall", "one", "two",
        "three", "four", "five", "six", "seven", "eight", "nine", "ten", "information",
        "network", "human", "history", "chapter", "book",
    }
    return {
...
def extract_en_paragraphs(xml: str) -> List[EnPara]:
    ...
    for match in BLOCK_RE.finditer(xml):
        ...
        if text:
            result.append(
                EnPara(
                    tag, attrs, inner, text, match.group(0),
                    match.start(), match.end(), attr_id(attrs), feature_set(text),
                )
            )
    return result
...
def _prefix_zh_ids(markup: str) -> str:
    ...
    return re.sub(
        r"\bhref=(['\"])#([^'\"]+)\1",
        lambda m: f'href={m.group(1)}#{m.group(2) if m.group(2).startswith("zh-") else "zh-" + m.group(2)}{m.group(1)}',
        markup,
    )
...
def extract_zh_units(xml: str) -> Tuple[List[ZhUnit], str]:
    ...
    for match in BLOCK_RE.finditer(body):
        ...
        units.append(
            ZhUnit(tag, prefixed_attrs, prefixed_inner, text, heading_html, heading_text, attr_id(attrs), feature_set(full_text))
        )
    ...
    if BR_RE.search(body) and len(BR_RE.findall(body)) > len(units):
        units = []
        body_match = re.search(r"<body\b[^>]*>(.*?)</body\s*>", body, flags=re.IGNORECASE | re.DOTALL)
        fallback_body = body_match.group(1) if body_match else body
        fallback_body = re.sub(r"<head\b[^>]*>.*?</head\s*>", "", fallback_body, flags=re.IGNORECASE | re.DOTALL)
        for fragment in BR_RE.split(fallback_body):
            ...
            if (
                ...
                or "\u203b" in text
                or re.fullmatch(r"\u7b2c?[\u4e00-\u4e95\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e\u96f6\u3007\u4e8c0-9]+\u7ae0?", text)
            ):
                continue
...
def align_interval(
    ...
    if dp[m][n] == inf:
        return [
            Group(en_offset + i, en_offset + i + 1, zh_offset + min(i, n - 1), zh_offset + min(i + 1, n), 9.0)
    ...
    while i or j:
        ...
        groups.append(
            Group(en_offset + start_i, en_offset + i, zh_offset + start_j, zh_offset + j, dp[i][j] - dp[start_i][start_j])
        )
        i, j = start_i, start_j
...
def load_overrides(path: Path | None) -> dict[str, List[dict]]:
    ...
    if not isinstance(items, list):
        raise ValueError("alignment overrides must be a JSON list or an object with an overrides list")
...
def split_sentences(text: str) -> List[str]:
    ...
    for token in tokens:
        ...
        if word in ABBREVIATIONS or re.fullmatch(r"[a-z]\.[a-z]", word) or re.search(r"\d\.\d$", buffered):
            continue
    ...
    for token in TAG_TOKEN_RE.split(inner):
        ...
        if token.startswith("<"):
            ...
            if token.startswith("</"):
                ...
                if name and name not in VOID_TAGS and not token.rstrip().endswith("/>"):
...
def extract_align_pairs(
    en_xhtml: str, zh_xhtml: str, overrides: dict[str, List[dict]] | None = None, chapter: str = "chapter"
) -> dict[str, Any]:
