...
class MN4WriteReceipt(BaseModel):
    """Receipt returned by MarginNote 4 after a remote write."""

    writeback_id: str = Field(min_length=1, max_length=64)
    remote_object_id: str = Field(min_length=1, max_length=128)
    content_hash: str = Field(min_length=64, max_length=64)
    written_at: float = Field(default_factory=time.time)


class MN4WritebackItem(BaseModel):
    """A locally queued write that awaits explicit user review."""
    ...
    status: MN4WritebackStatus = "pending_confirmation"
