...
_MN4_WRITEBACK_TRANSITIONS: dict[MN4WritebackStatus, set[MN4WritebackStatus]] = {
    "pending_confirmation": {"approved", "rejected"},
    "approved": {"applying"},
    "applying": {"applied", "failed", "conflicted"},
    "failed": {"pending_confirmation"},
    "conflicted": {"pending_confirmation"},
    "applied": set(),
    "rejected": set(),
}
...
class ImmersiveReadingService:
    ...
    def list_mn4_writebacks(self) -> list[MN4WritebackItem]:
        data = _read_json(self._mn4_writebacks_path(), [])
        if not isinstance(data, list):
            raise ValueError("Invalid MarginNote 4 writeback store")
        items: list[MN4WritebackItem] = []
        for index, item in enumerate(data):
            try:
                items.append(MN4WritebackItem.model_validate(item))
            except Exception as exc:
                raise ValueError(f"Invalid MarginNote 4 writeback at index {index}") from exc
        items.sort(key=lambda item: item.created_at, reverse=True)
        return items

    def _save_mn4_writebacks(self, items: list[MN4WritebackItem]) -> None:
        payload = sorted(
            (item.model_dump(mode="json") for item in items),
            key=lambda item: item["created_at"],
            reverse=True,
        )
        _write_json(self._mn4_writebacks_path(), payload)
    ...
    def create_mn4_writeback(
        ...
        *,
        ...
        """Create or return an idempotent pending MarginNote 4 write."""
        items = self.list_mn4_writebacks()
        existing = next((item for item in items if item.idempotency_key == idempotency_key), None)
        if existing is not None:
            if existing.content_hash != content_hash:
                raise ValueError("MarginNote 4 idempotency key conflicts with another content hash")
            return existing
        ...
        self._save_mn4_writebacks([*items, item])
        return item

    @staticmethod
    def _is_mn4_source(document_id: str) -> bool:
        return "mn4" in document_id.casefold()

    @staticmethod
    def _configured_model_name() -> str:
        try:
            return str(getattr(get_llm_config(), "model", "") or "")
        except LLMConfigError:
            return ""

    def queue_mn4_word_writeback(self, entry: VocabEntry) -> MN4WritebackItem | None:
        if not entry.document_id or not self._is_mn4_source(entry.document_id):
            return None
        digest = hashlib.sha256(f"{entry.word}\n{entry.context_en}".encode("utf-8")).hexdigest()
        return self.create_mn4_writeback(
            source_type="word",
            source_object_id=entry.id,
            content_hash=digest,
            idempotency_key=f"word:{entry.id}:{digest}",
            model=self._configured_model_name(),
        )

    def queue_mn4_translation_writeback(
        self, *, source_text: str, document_id: str, source_object_id: str
    ) -> MN4WritebackItem | None:
        if not document_id or not self._is_mn4_source(document_id):
            return None
        digest = hashlib.sha256(source_text.encode("utf-8")).hexdigest()
        return self.create_mn4_writeback(
            source_type="translation",
            source_object_id=source_object_id or document_id,
            content_hash=digest,
            idempotency_key=f"translation:{source_object_id or document_id}:{digest}",
            model=self._configured_model_name(),
        )

    def update_mn4_writeback_status(
        self, writeback_ids: list[str], status: MN4WritebackStatus
    ) -> int:
        if status not in _MN4_WRITEBACK_TRANSITIONS:
            raise ValueError("Invalid MarginNote 4 writeback status")
        requested = set(writeback_ids)
        items = self.list_mn4_writebacks()
        now = time.time()
        updated = 0
        for item in items:
            if item.id not in requested or status not in _MN4_WRITEBACK_TRANSITIONS[item.status]:
                continue
            item.status = status
            item.updated_at = now
            updated += 1
        if updated:
            self._save_mn4_writebacks(items)
        return updated

    def pull_mn4_writebacks(self) -> list[MN4WritebackItem]:
        items = self.list_mn4_writebacks()
        pulled: list[MN4WritebackItem] = []
        now = time.time()
        for item in items:
    ...
    def submit_mn4_receipts(self, receipts: list[MN4WriteReceipt]) -> int:
        items = self.list_mn4_writebacks()
        by_id = {item.id: item for item in items}
        now = time.time()
        updated = 0
        for receipt in receipts:
            item = by_id.get(receipt.writeback_id)
            if item is None or item.status != "applying":
                continue
            item.receipt = receipt
            item.status = "applied" if receipt.content_hash == item.content_hash else "conflicted"
            item.updated_at = now
            updated += 1
        if updated:
            self._save_mn4_writebacks(items)
    ...
    async def add_word(
        ...
        try:
            self.queue_mn4_word_writeback(entry)
        except Exception:
            logger.exception("Failed to queue MarginNote 4 writeback for word %s", entry.id)
        return entry
