...
class ImportResolver:
    ...
    def preview(self, inputs: Sequence[str]) -> list[dict[str, Any]]:
        ...
        if len(candidates) > 500:
            raise MediaStoreError(
                "An import preview may contain at most 500 items. Split the import."
            )
            ...
            if not canonical_id or media_type not in {kind.value for kind in MediaType}:
                results.append(
                    self._unrecognized(
                        ...
                        "Missing canonical media identity.",
                    )
                )
                continue
            results.append(
    ...
    def _preview_opml(self, raw: str) -> list[dict[str, Any]]:
        ...
        for index, outline in enumerate(root.findall(".//outline")):
            ...
            if not feed.startswith(("http://", "https://")):
                results.append(
                    self._unrecognized(index, feed or title, "OPML outline has no network RSS URL.")
                )
                continue
            results.append(
                self._candidate(
        ...
        return results or [
            self._unrecognized(0, "OPML", "No podcast feeds were found in this OPML document.")
        ]
    ...
    def _preview_m3u(self, raw: str) -> list[dict[str, Any]]:
        ...
        for index, line in enumerate(raw.splitlines()):
            ...
            title = ""
        return results or [
            self._unrecognized(0, "M3U", "No network media URLs were found in this M3U file.")
        ]
    ...
    def _preview_scalar(self, raw: str, index: int) -> dict[str, Any]:
        ...
        if mbid_match:
            ...
            if kind == "recording":
                return self._candidate(
                    ...
                    source=raw,
                    status=ImportCandidateStatus.MATCHED,
                    canonical_id=f"mbid:recording:{identifier}",
                    media_type=MediaType.MUSIC.value,
                    title=f"MusicBrainz recording {identifier}",
                    detail="Recording metadata will be refreshed from MusicBrainz.",
                    source_url=raw,
                )
            return self._candidate(
                index,
                source=raw,
                status=ImportCandidateStatus.NEEDS_CONFIRMATION,
                canonical_id=f"mbid:{kind}:{identifier}",
                media_type=MediaType.MUSIC.value,
                title=f"MusicBrainz {kind} {identifier}",
                detail=f"Expand this MusicBrainz {kind} after confirmation.",
                source_url=raw,
            )
        ...
        if parsed.scheme in {"http", "https"}:
            ...
            if host.endswith("youtube.com") or host == "youtu.be":
                ...
                if playlist_id:
                    return self._candidate(
                        ...
                        source=raw,
                        status=ImportCandidateStatus.NEEDS_CONFIRMATION,
                        canonical_id=f"youtube:playlist:{playlist_id}",
                        media_type=MediaType.MUSIC.value,
                        title="YouTube playlist",
                        detail="Playlist items will be resolved after confirmation.",
                        source_url=raw,
                    )
                if _YOUTUBE_ID_RE.fullmatch(video_id):
                    return self._candidate(
                        ...
                        source=raw,
                        status=ImportCandidateStatus.MATCHED,
                        canonical_id=f"youtube:video:{video_id}",
                        media_type=MediaType.MUSIC.value,
                        title=f"YouTube media {video_id}",
                        detail="Audio playback uses configured Invidious or Yattee sources.",
                        source_url=raw,
                    )
            if host == "podcastindex.org" and parsed.path.rstrip("/").split("/")[-1].isdigit():
                ...
                return self._candidate(
                    ...
                    source=raw,
                    status=ImportCandidateStatus.NEEDS_CONFIRMATION,
                    canonical_id=f"podcastindex:{identifier}",
                    media_type=MediaType.PODCAST.value,
                    title=f"Podcast Index {identifier}",
                    detail="Podcast metadata will be resolved after confirmation.",
                    source_url=raw,
                )
            ...
            return self._candidate(
                ...
                source=raw,
                status=ImportCandidateStatus.NEEDS_CONFIRMATION,
                canonical_id=_digest(raw, "url"),
                media_type=MediaType.MUSIC.value,
                title=_title_from_url(raw),
                detail="Confirm the media URL before adding it.",
                source_url=raw,
            )
        ...
        if " - " in raw:
            ...
            return self._candidate(
                ...
                source=raw,
                status=ImportCandidateStatus.NEEDS_CONFIRMATION,
                canonical_id=_digest(raw, "text"),
                media_type=MediaType.MUSIC.value,
                title=title.strip(),
                creator=creator.strip(),
                detail="Confirm the title and artist match before adding it.",
            )
        return self._unrecognized(
            index,
            raw,
            "Recognized formats include MusicBrainz, YouTube, RSS, OPML, M3U and LinguaWave JSON.",
        )
    ...
    def _candidate(
        ...
        *,
        ...
        return {
            "candidate_id": _candidate_id(index, source),
            "status": status.value,
            "source": source,
            "canonical_id": canonical_id,
            "media_type": media_type,
    ...
    def _unrecognized(self, index: int, source: str, detail: str) -> dict[str, Any]:
        return self._candidate(
            index, source=source, status=ImportCandidateStatus.UNRECOGNIZED, detail=detail
        )
    ...
    def _with_duplicate_status(self, candidate: dict[str, Any]) -> dict[str, Any]:
        ...
        if canonical_id and self.store.get_media_by_canonical_id(canonical_id) is not None:
            return {
                **candidate,
                "status": ImportCandidateStatus.DUPLICATE.value,
...
class HomeFeedAssembler:
    ...
    def _card_for_media(media: MediaItem, *, reason: str = "") -> HomeCard:
        ...
        return HomeCard(
            card_id=f"media:{media.item_id}",
            title=media.title,
            subtitle=media.creator or media.collection_title,
    ...
    def build(self, *, content_filter: str = "all", offline: bool = False) -> list[HomeSection]:
        ...
        in_progress = [
            item
            for item in media
            if item.playback_position_seconds > 0 and item.learning_progress < 1
        ]
        learning = [
            item
            for item in media
            if item.learning_progress < 1
            and item.item_id in {row["item_id"] for row in self._with_timed_text(media)}
        ]
        ...
        if offline and downloaded:
            sections.append(
                HomeSection(
                    ...
                    title="离线可听",
                    display="shelf",
                    cards=[self._card_for_media(item, reason="已下载") for item in downloaded[:12]],
                )
            )
        sections.append(
            HomeSection(
                section_id="quick_start",
                kind=HomeSectionKind.QUICK_START,
                title="快速开始",
                display="grid",
                cards=quick_cards,
            )
        ...
        if in_progress:
            sections.append(
                HomeSection(
                    section_id="continue",
                    kind=HomeSectionKind.CONTINUE_PLAYING,
                    title="继续播放",
                    display="hero",
                    cards=[self._card_for_media(in_progress[0], reason="从上次的位置继续")]
                    + [self._card_for_media(item) for item in in_progress[1:8]],
                )
            )
        playlists = self.store.list_playlists()
        playlist_cards = [
            HomeCard(
                card_id=f"playlist:{playlist['playlist_id']}",
                title=str(playlist["name"]),
                subtitle=f"{playlist['item_count']} 项",
        ...
        if playlist_cards:
            sections.append(
                HomeSection(
                    section_id="playlists",
                    kind=HomeSectionKind.PLAYLISTS,
                    title="你的播放清单",
                    display="shelf",
                    cards=playlist_cards,
                )
            )
        ...
        if subscription_cards:
            sections.append(
                HomeSection(
                    ...
                    kind=HomeSectionKind.SUBSCRIPTION_UPDATES,
                    title="订阅有更新",
                    display="shelf",
        ...
        recommendation_cards = [
            self._card_for_media(item, reason="来自你的资料库和学习偏好")
            for item in media[:12]
            if item not in in_progress
        ]
        if recommendation_cards:
            sections.append(
                HomeSection(
                    section_id="recommendations",
                    kind=HomeSectionKind.RECOMMENDATIONS,
                    title="为你推荐",
                    display="shelf",
        ...
        if learning:
            sections.append(
                HomeSection(
                    section_id="learning",
                    kind=HomeSectionKind.CONTINUE_LEARNING,
                    title="继续学习",
                    display="shelf",
                    cards=[
                        self._card_for_media(item, reason="还有可学习的文字片段")
                        for item in learning[:12]
                    ],
                )
            )
    ...
    def _quick_start(
        self,
        in_progress: list[MediaItem],
        media: list[MediaItem],
        downloaded: list[MediaItem],
        learning: list[MediaItem],
    ) -> list[HomeCard]:
        cards: list[HomeCard] = []
        choices: list[tuple[str, str, MediaItem | None]] = [
            ...
            (
                "每日音乐",
                "为今天准备",
                next((item for item in media if item.media_type == MediaType.MUSIC), None),
            ),
            (
                "最新播客",
                "订阅节目更新",
                next((item for item in media if item.media_type == MediaType.EPISODE), None),
            ),
    ...
    def _subscription_cards(self, media: Sequence[MediaItem]) -> list[HomeCard]:
        ...
        matches = [
            ...
            and any(item.source_url.startswith(source) for source in source_urls)
        ]
    ...
    def _filter(items: list[MediaItem], content_filter: str) -> list[MediaItem]:
        ...
        if content_filter == "podcast":
            return [
                item for item in items if item.media_type in {MediaType.PODCAST, MediaType.EPISODE}
            ]
...
class MediaService:
    def __init__(
        self, store: MediaStore | None = None, catalog: CatalogStore | None = None
    ) -> None:
    ...
    def bootstrap(self) -> dict[str, Any]:
        ...
        return {
            ...
            "playlists": self.store.list_playlists(),
            "providers": {
                "musicbrainz": "cache_ready",
                "listenbrainz": "optional",
                "podcast_index": "optional",
                "invidious": "optional",
                "yattee": "optional",
            },
        ...
        if category in {"recommended", "music"}:
            ...
            shelves.append(
                DiscoveryShelf(
                    ...
                    category="music",
                    cards=[
                        HomeFeedAssembler._card_for_media(item, reason="来自最近播放与收藏")
                        for item in music[:12]
                    ],
                )
            )
            shelves.append(
                DiscoveryShelf(
                    shelf_id="music_new",
                    title="新发行与热门 Recording",
                    category="music",
                    cards=[
                        HomeFeedAssembler._card_for_media(item, reason="目录缓存")
                        for item in music[12:24]
                    ],
                )
            )
        if category in {"recommended", "podcast"}:
            podcasts = [
                item for item in media if item.media_type in {MediaType.PODCAST, MediaType.EPISODE}
            ]
            shelves.append(
                DiscoveryShelf(
                    shelf_id="podcast_trending",
                    title="播客热门与新节目",
                    category="podcast",
                    cards=[
                        HomeFeedAssembler._card_for_media(item, reason="订阅主题")
                        for item in podcasts[:12]
                    ],
                )
            )
        if category in {"recommended", "topic"}:
            shelves.append(
                DiscoveryShelf(
                    ...
                    cards=[
                        HomeCard(
                            card_id=f"topic:{name}",
                            title=name,
                            subtitle="浏览并个性化排序",
                            action="open_topic",
                        )
                        for name in ("科技", "文化", "语言", "新闻", "流行", "古典")
                    ],
                )
            )
    ...
    def add_local_item(self, payload: dict[str, Any]) -> MediaItem:
        canonical_id = str(
            ...
            or _digest(str(payload.get("source_url") or payload.get("title") or ""), "local")
        )
        item = MediaItem(
            item_id=media_id_for(canonical_id),
            canonical_id=canonical_id,
            media_type=MediaType.LOCAL,
            title=str(payload.get("title") or "本地音频"),
    ...
    def create_playlist(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        rule = (
            SmartPlaylistRule.model_validate(payload.get("rule") or {})
    ...
    def playlist_detail(self, playlist_id: str) -> dict[str, Any]:
        return {
            **self.store.get_playlist_row(playlist_id),
            "items": self.store.get_playlist_items(playlist_id),
        }
    ...
    def update_playlist(self, playlist_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return self.store.update_playlist(
            playlist_id,
    ...
    def add_playlist_items(
        ...
        *,
        ...
        return self.store.add_playlist_items(
            playlist_id, canonical_ids, added_from=added_from, note=note
        )

    # -- imports ---------------------------------------------------------

    def preview_import(
        self, inputs: Sequence[str], *, target_playlist_id: str | None, playlist_name: str
    ) -> dict[str, Any]:
        if target_playlist_id:
            self.store.get_playlist_row(target_playlist_id)
        candidates = ImportResolver(self.store).preview(inputs)
        return self.store.save_import_preview(
            candidates=candidates,
            target_playlist_id=target_playlist_id,
            suggested_playlist_name=playlist_name or "导入的播放清单",
        )
    ...
    def apply_import(
        ...
        *,
        ...
        if replay is not None:
            return ImportResult(
                replay, is_background=replay.get("status") == ImportJobStatus.QUEUED.value
            )
        preview = self.store.get_import_preview(preview_token)
        candidates = preview["candidates"]
        selected = set(
            selected_candidate_ids or [str(candidate["candidate_id"]) for candidate in candidates]
        )
        accepted = [
            candidate
            ...
            in {ImportCandidateStatus.MATCHED.value, ImportCandidateStatus.NEEDS_CONFIRMATION.value}
        ]
        if not accepted:
    ...
    def run_import_job(
        self, job_id: str, *, target_playlist_id: str | None = None, playlist_name: str = ""
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return self.store.update_import_job(
                ...
                imported=int(applied.get("imported_item_count") or 0),
                failed=int(applied.get("failed_item_count") or 0),
            )
        except Exception as exc:
            return self.store.update_import_job(
                job_id, status=ImportJobStatus.FAILED, error=str(exc)
            )

    def _apply_import_now(
        self,
        ...
        if playlist_id:
            ...
            if playlist["kind"] not in {
                PlaylistKind.MANUAL.value,
                PlaylistKind.EXTERNAL_IMPORT.value,
            }:
                raise MediaStoreError("Imports require a manual playlist target.")
        else:
            playlist = self.store.create_playlist(
                name=playlist_name
                or str(preview.get("suggested_playlist_name") or "导入的播放清单"),
                description="通过 LinguaWave 导入",
                kind=PlaylistKind.EXTERNAL_IMPORT,
            )
        ...
        for candidate in accepted:
            try:
                ...
                failures += 1
        if canonical_ids:
            self.store.add_playlist_items(
                str(playlist_id), canonical_ids, added_from="import", note=""
            )
        return {
            "playlist_id": playlist_id,
            "status": "completed",
            "imported_item_count": len(canonical_ids),
            "failed_item_count": failures,
            "playlist": self.playlist_detail(str(playlist_id)),
        }
    ...
    def playback(self, item_id: str) -> PlaybackDescriptor:
        ...
        if item.media_type in {MediaType.PODCAST, MediaType.EPISODE} and item.source_url:
            return PlaybackDescriptor(
                kind="audio", provider="rss", stream_url=item.source_url, supports_video=False
            )
        ...
        if source:
            return PlaybackDescriptor(
                ...
                stream_url=str(source["source_url"]),
                supports_video=True,
            )
        return PlaybackDescriptor(
            kind="unavailable", reason="Resolve an Invidious or Yattee source before playback."
        )

    def resolve_feed(
        self, source_url: str, *, title: str = "", language: str = ""
    ) -> dict[str, Any]:
