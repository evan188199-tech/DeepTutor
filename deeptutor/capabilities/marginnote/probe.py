...
def _blocked(kind: str, folder: Path, error: str, recover: list[str], status: str = "disconnected") -> AdapterDiagnostics:
...
def probe_marginnote(
    ...
    *,
    ...
    if kind not in PUBLIC_ADAPTERS:
        ...
        diagnostics = _blocked(kind, folder, f"Folder does not exist: {folder.name}", ["choose_existing_export"])
