...
All methods return plain dicts / strings so the sync engine and tests can
work with mock clients easily.
"""
...
class GitHubClient:
    ...
    async def get_tree(
        ...
        *,
        ...
        """Return tree entries that match ``glob`` under ``path_prefix``."""
        tree_sha = branch  # recursive tree by branch/SHA ref works on GitHub
        data = await self._request_json(
            "GET",
    ...
    async def compare_commits(
        ...
        """Return the list of file-level changes between two SHAs."""
        data = await self._request_json(
            "GET",
    ...
    async def download_file(self, repo: str, path: str, ref: str) -> bytes:
        """Download raw file content via the raw endpoint."""
        url = f"https://raw.githubusercontent.com/{repo}/{ref}/{path}"
        content = await self._request_bytes(url)
        if len(content) > MAX_FILE_BYTES:
            logger.warning(
                "GitHub file %s/%s@%s truncated: %d bytes > %d limit",
                repo, path, ref, len(content), MAX_FILE_BYTES,
            )
...
def _match_glob(path: str, glob: str) -> bool:
    """Lightweight glob matcher — supports ``*.md`` style patterns.

    Uses :mod:`fnmatch` for correctness.
    """
    import fnmatch
