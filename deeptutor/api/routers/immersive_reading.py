...
class TranslateRequest(BaseModel):
    ...
    source_object_id: str = Field(default="", max_length=128)
...
class MN4WritebackIdsRequest(BaseModel):
    writeback_ids: list[str] = Field(min_length=1, max_length=100)


class MN4WritebackReceiptsRequest(BaseModel):
    receipts: list[MN4WriteReceipt] = Field(min_length=1, max_length=100)
...
async def translate(request: TranslateRequest) -> dict:
    try:
        service = get_immersive_reading_service()
        translated = await service.translate(request.text, request.target_language)
        service.queue_mn4_translation_writeback(
...
# ── MarginNote 4 writeback review ─────────────────────────────────────────


@router.get("/mn4/writebacks")
async def list_mn4_writebacks() -> dict[str, Any]:
    try:
        items = get_immersive_reading_service().list_mn4_writebacks()
    except ValueError as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
    return {"writebacks": [item.model_dump(mode="json") for item in items]}


@router.post("/mn4/writebacks/approve")
async def approve_mn4_writebacks(request: MN4WritebackIdsRequest) -> dict[str, Any]:
    try:
        count = get_immersive_reading_service().update_mn4_writeback_status(
            request.writeback_ids, "approved"
        )
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
@router.post("/mn4/writebacks/reject")
async def reject_mn4_writebacks(request: MN4WritebackIdsRequest) -> dict[str, Any]:
    try:
        count = get_immersive_reading_service().update_mn4_writeback_status(
            request.writeback_ids, "rejected"
        )
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
async def pull_mn4_writebacks() -> dict[str, Any]:
    items = get_immersive_reading_service().pull_mn4_writebacks()
    return {"pending_items": [item.model_dump(mode="json") for item in items]}


@router.post("/mn4/writebacks/receipt")
async def submit_mn4_receipts(request: MN4WritebackReceiptsRequest) -> dict[str, Any]:
    count = get_immersive_reading_service().submit_mn4_receipts(request.receipts)
