...
def detect_video_provider(url: str) -> VideoProvider | None:
    ...
    if host in {"youtube.com", "www.youtube.com", "m.youtube.com", "music.youtube.com"}:
        return (
            "youtube"
            if path == "/watch" or path.startswith(("/watch/", "/shorts/", "/live/", "/embed/"))
            else None
        )
    if host == "youtu.be":
...
async def learn_video(
    ...
    *,
    ...
    if provider == "youtube":
        ...
        try:
            ...
            return VideoLearningOutcome(
                provider="youtube", url=url, error=f"Invalid video data: {exc}"
            )
        except Exception as exc:  # pragma: no cover - defensive provider boundary
            return VideoLearningOutcome(
                provider="youtube", url=url, error=f"Unexpected video failure: {exc}"
            )
    ...
    try:
        ...
        return VideoLearningOutcome(
            provider="bilibili", url=url, error=f"Invalid video data: {exc}"
        )
    except Exception as exc:  # pragma: no cover - defensive boundary for provider changes
        return VideoLearningOutcome(
            provider="bilibili", url=url, error=f"Unexpected video failure: {exc}"
        )
...
async def _learn_youtube(
    ...
    *,
    ...
    if request.entry_time_seconds:
        selected = [
            row for row in transcript_segments if context_start <= row["start"] <= context_end
        ]
...
async def _invidious_transcript(
    ...
    *,
    ...
    if not isinstance(captions, list) or not captions:
        try:
            payload = await _invidious_request(
                client, f"{base_url}/companion/api/v1/captions/{video_id}"
            )
    ...
    ranked = _select_youtube_caption(captions, preferred_language)
    candidates: list[dict[str, Any]] = (
        [ranked]
        if ranked
        else (
            [c for c in captions if isinstance(c, dict)]
            or [{"languageCode": preferred_language or "en"}]
        )
    )
    ...
    for selected in candidates:
        ...
        raw_url = str(selected.get("url") or "")

        urls_to_try: list[str] = []
        if raw_url:
            urls_to_try.append(urljoin(base_url + "/", raw_url.lstrip("/")))
            if "/api/v1/captions/" in raw_url:
                urls_to_try.append(
                    urljoin(
                        base_url + "/",
                        raw_url.replace("/api/v1/captions/", "/companion/api/v1/captions/").lstrip(
                            "/"
                        ),
                    )
                )
        ...
        for q in queries:
            urls_to_try.append(f"{base_url}/companion/api/v1/captions/{video_id}?{q}")
            urls_to_try.append(f"{base_url}/api/v1/transcripts/{video_id}?{q}")
            if language:
                urls_to_try.append(f"{base_url}/api/v1/transcripts/{video_id}?{q}&autogen")
            urls_to_try.append(f"{base_url}/api/v1/captions/{video_id}?{q}")

        urls_to_try.append(f"{base_url}/companion/api/v1/captions/{video_id}")
        urls_to_try.append(f"{base_url}/api/v1/transcripts/{video_id}")
        urls_to_try.append(f"{base_url}/api/v1/captions/{video_id}")
            ...
            try:
                ...
                if "json" in content_type:
                    ...
                    if isinstance(rows, list):
                        cues = _normalize_transcript_segments(rows)
                        if cues:
    ...
    for language in priorities:
        row = next(
            (
                item
                for item in rows
                if str(item.get("languageCode") or item.get("language_code") or "") == language
            ),
            None,
        )
        if row:
...
def _context_window(
    ...
    *,
    ...
    if segments:
            ...
            int(
                max(row["end"] for row in segments if row["start"] <= entry_time_seconds)
                if any(row["start"] <= entry_time_seconds for row in segments)
                else end
            ),
        )
    return start, end
    ...
    for item in value:
        ...
        try:
            start = max(
                0,
                int(
                    float(item.get("start_time") or item.get("startTime") or item.get("start") or 0)
                ),
            )
        except (TypeError, ValueError):
            continue
...
def _format_youtube_markdown(
    *,
    ...
    if entry_time_seconds:
        metadata.append(f"- Current position: [{_format_timestamp(entry_time_seconds)}]({url})")
    if language:
...
def _parse_webvtt(text: str) -> list[dict[str, Any]]:
    ...
    for match in pattern.finditer(text):
        ...
        if body:
            rows.append(
                {
                    "start": _vtt_timestamp(match.group("start")),
                    "end": _vtt_timestamp(match.group("end")),
                    "text": body,
                }
            )
    return rows
...
async def _read_youtube_audio(client: Any, url: str) -> tuple[bytes, str]:
    ...
    for _ in range(MAX_INVIDIOUS_REDIRECTS + 1):
        ...
        async with client.stream("GET", current, follow_redirects=False) as response:
            ...
            final_host = (
                urlparse(str(getattr(response, "url", "") or current)).hostname or ""
            ).lower()
            ...
            async for chunk in response.aiter_bytes():
                ...
                if total > MAX_AUDIO_BYTES:
                    raise ValueError(
                        "YouTube audio exceeds the 32 MB no-download preprocessing cap"
                    )
                chunks.append(chunk)
            audio = b"".join(chunks)
            if not audio:
...
async def _learn_bilibili(
    ...
    *,
    ...
    player = await _bilibili_json(
        client,
        f"https://api.bilibili.com/x/player/v2?{video_id.kind}={video_id.value}&cid={cid}",
        headers,
        validator,
    )
    ...
    *,
    ...
    try:
        ...
        async with client_factory(
            timeout=DEFAULT_TIMEOUT_S, user_agent=DEFAULT_USER_AGENT
        ) as client:
            metadata = await _bilibili_json(
                client,
                f"https://api.bilibili.com/x/web-interface/view?{video_id.kind}={video_id.value}",
            ...
            audio_url = _select_bilibili_audio_url(
                _dict(play.get("data")).get("dash"), host_validator
            )
...
async def _read_bilibili_audio(
    ...
    *,
    ...
    async with client.stream("GET", url, headers=headers, follow_redirects=True) as response:
        ...
        final_host = (
            (urlparse(str(getattr(response, "url", "") or url)).hostname or "").lower().rstrip(".")
        )
...
def _normalize_transcript_segments(segments: Any) -> list[dict[str, Any]]:
    ...
    for item in segments:
        ...
        try:
            start = max(
                0.0,
                float(entry.get("from") or entry.get("start") or entry.get("startMs", 0) / 1000),
            )
            end = float(entry.get("to") or entry.get("end") or entry.get("endMs", 0) / 1000)
            if end <= start:
                end = start + float(entry.get("duration") or entry.get("durationMs", 0) / 1000 or 0)
        except (TypeError, ValueError):
            start, end = 0.0, 0.0
...
async def _bilibili_json(
    ...
    async with client.stream("GET", safe_url, headers=headers, follow_redirects=True) as response:
