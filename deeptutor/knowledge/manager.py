...
class KnowledgeBaseManager:
    ...
    # ------------------------------------------------------------------
    # GitHub source management — mirrors linked_folders pattern
    # ------------------------------------------------------------------
    ...
    def add_github_source(
        ...
        Returns the source info dict (idempotent — re-adding the same
        repo+branch+path returns the existing entry).
        """
        if kb_name not in self.list_knowledge_bases():
            raise ValueError(f"Knowledge base not found: {kb_name}")

        # Normalise repo to "owner/name" (strip trailing slash, .git, URL prefix)
    ...
    def remove_github_source(self, kb_name: str, source_id: str) -> bool:
        """Remove a GitHub source from a KB. Returns True if found."""
        if kb_name not in self.list_knowledge_bases():
            raise ValueError(f"Knowledge base not found: {kb_name}")
    ...
    def get_github_sources(self, kb_name: str) -> list[dict]:
    ...
    def update_github_source_state(
        self,
        kb_name: str,
        source_id: str,
        **fields: Any,
    ...
    def get_all_github_sources(self) -> list[tuple[str, dict]]:
        """Scan every KB and return ``(kb_name, source_dict)`` pairs.

        Used by the background sync service to find all eligible sources.
        """
    ...
    @staticmethod
    def _read_kb_metadata(metadata_file: Path) -> dict:
