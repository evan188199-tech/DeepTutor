  ...
  return JSB.defineClass(
    "DeepTutorMarginNote4 : JSExtension",
    {
      ...
      controlDeepTutor: function(sender) {
        var clipboard = dtText(UIPasteboard.generalPasteboard().string).trim();
      ...
      pairFromClipboard: function() {
        var clipboard = dtText(UIPasteboard.generalPasteboard().string).trim();
        var parts = clipboard.split("|");
        if (parts.length !== 2) {
          Application.sharedInstance().alert(
            "Copy 'DeepTutor server URL|pairing code' to the clipboard, then tap the DeepTutor command."
          );
          return;
        }
      ...
      collectObjects: function() {
        var model = MbModelTool.sharedInstance();
        var objects = [];
        var documents = {};
        var rawDocuments = dtValue(model.allDocuments(), []);
        ...
        var notebooks = dtValue(model.allNotebooks(), []);
