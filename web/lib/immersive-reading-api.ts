...
export type MN4WritebackStatus =
...
export const immersiveReadingApi = {
  ...
  translate: (
    ...
    signal?: AbortSignal,
    source?: { documentId?: string; sourceObjectId?: string },
  ) =>
  ...
  sectionVocabularyDifficulty: (documentId: string, sectionId: string) =>
    request<VocabularyDifficultyPreview>(
      `/documents/${encodeURIComponent(documentId)}/sections/${encodeURIComponent(
        sectionId,
      )}/vocabulary-difficulty`,
    ),
  bilingualVocabularyDifficulty: (pairingId: string, chapterId: string) =>
    request<VocabularyDifficultyPreview>(
      `/bilingual/${encodeURIComponent(pairingId)}/section/${encodeURIComponent(
        chapterId,
      )}/vocabulary-difficulty`,
    ),
  characterGraph: (
...
export const mn4WritebackApi = {
  ...
  approve: (writebackIds: string[]) =>
    request<{ approved_count: number }>("/mn4/writebacks/approve", {
      method: "POST",
      body: JSON.stringify({ writeback_ids: writebackIds }),
    }),
  reject: (writebackIds: string[]) =>
    request<{ rejected_count: number }>("/mn4/writebacks/reject", {
      method: "POST",
      body: JSON.stringify({ writeback_ids: writebackIds }),
    }),
  pull: () =>
    request<{ pending_items: MN4WritebackItem[] }>("/mn4/writebacks/pull", {
      method: "POST",
    }),
  submitReceipts: (receipts: MN4WriteReceipt[]) =>
    request<{ processed_count: number }>("/mn4/writebacks/receipt", {
      method: "POST",
