...
export interface MaterialInfo {
  ...
  /** Existing per-locator source/translation data is available. */
...
export async function listMaterials(): Promise<MaterialInfo[]> {
  return unwrap(await apiFetch(apiUrl(`${BASE}/materials`), { cache: 'no-store' }))
}
...
export async function getBilingualUnit(
  ...
  return unwrap(
    await apiFetch(apiUrl(`${BASE}/materials/${materialId}/units/${locator}/bilingual`), {
      cache: 'no-store',
    })
  )
}
...
export async function runReadingExtension(
  ...
  return unwrap(
    await apiFetch(
      apiUrl(`${BASE}/materials/${materialId}/extensions/${extensionId}/actions/${action}`),
      {
        method: 'POST',
...
export async function getReadingPosition(materialId: string): Promise<ReadingPosition> {
  return unwrap(
...
export async function listAnnotations(materialId: string): Promise<AnnotationItem[]> {
  return unwrap(
...
export async function deleteAnnotation(materialId: string, annotationId: string): Promise<void> {
  await unwrap(
    await apiFetch(apiUrl(`${BASE}/materials/${materialId}/annotations/${annotationId}`), {
      method: 'DELETE',
    })
  )
}
...
export async function fetchExport(
  ...
  const response = await apiFetch(apiUrl(`${BASE}/materials/${materialId}/export?fmt=${fmt}`))
  if (!response.ok) {
 ...
 */
export function filenameFromDisposition(header: string | null, fallback = 'export'): string {
