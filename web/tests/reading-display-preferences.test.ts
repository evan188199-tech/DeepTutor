...
test('text reader exposes persistent display preferences', () => {
  ...
  assert.deepEqual([DEFAULT_FONT_SIZE, MIN_FONT_SIZE, MAX_FONT_SIZE], [17, 12, 28])
  assert.deepEqual([DEFAULT_LINE_WIDTH, MIN_LINE_WIDTH, MAX_LINE_WIDTH], [84, 48, 104])
  assert.match(reader, /browserStorage\.writeRaw\(\s*["\x27]local["\x27]/)
})
...
test('stored preferences are bounded and malformed values fall back', () => {
  assert.deepEqual(
    ...
    { fontSize: 28, lineWidth: 48, serif: false, readerTheme: 'night', bilingual: false }
  )
  assert.deepEqual(normaliseReaderDisplayPreferences({ readerTheme: 'invalid' }), {
    fontSize: 17,
    lineWidth: 84,
    serif: true,
    readerTheme: 'auto',
...
test('keyboard zoom is handled only while the reader is active', () => {
  ...
  assert.equal(readerDisplayShortcut({ ...input, readerHovered: true }), 'increase')
  assert.equal(readerDisplayShortcut({ ...input, key: '-', readerFocused: true }), 'decrease')
  assert.equal(readerDisplayShortcut({ ...input, key: '0', readerHovered: true }), 'reset')
  assert.equal(readerDisplayShortcut({ ...input, modifier: false, readerHovered: true }), null)
  assert.match(reader, /root\?\.matches\(["\x27]:hover["\x27]\)/)
