...
test('the build wrapper restores every generated checked-in input', () => {
  ...
  assert.match(source, /stdio: "inherit"/, 'wrapper must preserve Next build diagnostics')
  assert.match(
    source,
...
test('the build wrapper makes standalone output self-contained', () => {
  ...
  assert.match(source, /const runtimeDistDir = path\.relative\(webRoot, distDir\)/)
  assert.match(source, /path\.join\(standaloneDir, "\.next", "static"\)/)
  assert.match(source, /path\.join\(standaloneDir, runtimeDistDir, "static"\)/)
  assert.match(source, /path\.join\(standaloneDir, "public"\)/)
  assert.match(
    source,
    /for \(const target of staticTargets\) \{\s*rmSync\(target/,
    'stale hashed assets must not survive a rebuild'
  )
