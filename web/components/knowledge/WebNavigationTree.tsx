...
export default function WebNavigationTree({
  ...
  // Default-expand top-level nodes that have children when data arrives.
  useEffect(() => {
  ...
  // Expand ancestors of the selected file so it's visible in the tree.
  useEffect(() => {
    ...
    for (const src of sources) {
      ...
      if (found) {
        setExpanded((prev) => {
          const next = new Set(prev);
          for (const node of found) next.add(node.id);
          return next;
        });
        break;
      }
