...
import type { AnnotationItem, BilingualGroup, UnitKind } from '@/lib/reading-api'
...
export function TextUnitView({
  ...
  const textSelectorToolsRef = useRef<{
    ...
    getQuoteContext: (range: Range, container: HTMLElement) => { prefix: string; suffix: string }
  } | null>(null)
  ...
  const cycleLineWidth = useCallback(() => {
    const nextWidth = LINE_WIDTH_STEPS.find(width => width > lineWidth) ?? MIN_LINE_WIDTH
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Translation disclosure stays local to the viewed section.
    ...
    void import('@recogito/text-annotator')
      .then(module => {
        ...
        const instance = module.createTextAnnotator(article, {
          ...
          style: annotation => {
            const properties = annotation.properties as
              { annotationId?: string; color?: string; kind?: string } | undefined
            ...
            return {
              fill: `rgb(${color})`,
              fillOpacity: properties?.annotationId === highlightedAnnotationId ? 0.8 : 0.55,
            }
          },
        })
        annotator = instance
        const rows = annotations
          ...
          .map(annotation => toRecogitoTextAnnotation(annotation, article.textContent ?? ''))
  ...
  }, [annotations, error, highlightedAnnotationId, loading, locator, onAnnotationClick, text])

  const pageHeadings = useMemo(() => extractReaderHeadings([text], locator), [locator, text])

  useEffect(() => {
    if (!isWebMarkdown) return
    const article = articleRef.current
    if (!article || loading || error) return
    article.querySelectorAll<HTMLElement>('h1,h2,h3,h4,h5,h6').forEach((element, index) => {
  ...
  useEffect(() => {
    ...
    container.scrollTo({
      top: Math.max(0, container.scrollTop + elementRect.top - containerRect.top - 24),
    })
  ...
  const handlePointerUp = useCallback(() => {
    ...
    const quote =
      selector && selector.quote.length <= 2000 ? selector.quote : cleanQuote(selection.toString())
    if (!quote) {
      onSelection(null)
      return
    }
    ...
    onSelection({
      ...
      selectors:
        selector && selector.quote === quote
          ...
          : [],
      anchor: last ? { x: last.left + last.width / 2, y: last.top } : { x: 0, y: 0 },
    })
    ...
    >
      <div className="flex items-center justify-between gap-2 overflow-x-auto border-b border-[var(--border)] px-2 py-2 sm:px-3">
        <div className="flex shrink-0 items-center gap-0.5">
          ...
          />
          <PreferenceButton label={t('Change line width')} icon={Rows3} onClick={cycleLineWidth} />
          ...
          <PreferenceButton
            ...
            onClick={() =>
              updatePreferences({
                readerTheme:
                  readerTheme === 'auto' ? 'sepia' : readerTheme === 'sepia' ? 'night' : 'auto',
              })
            }
          />
          {bilingualAvailable ? (
            ...
            {canReveal && (
              <div className="mt-2">
                <button
                  ...
                  className="rounded-md border border-[var(--border)] px-2 py-1 text-[11px] text-[var(--muted-foreground)] transition hover:bg-[var(--muted)] hover:text-[var(--foreground)]"
                >
                  {expanded ? '收起中文' : '显示中文'}
                </button>
                {expanded && (
                  <div className="mt-2 border-l-2 border-[var(--primary)]/40 pl-3 text-[var(--muted-foreground)]">
                    <RichMarkdownRenderer
...
function TextWithHeadings({ text, headings }: { text: string; headings: ReaderHeading[] }) {
  ...
  return (
    <>
      {lines.map((line, lineIndex) => {
        ...
        if (line.heading) {
          const Heading = `h${line.heading.level}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6'
          const titleOffset = line.text.indexOf(line.heading.title)
          const markerPrefix = titleOffset >= 0 ? line.text.slice(0, titleOffset) : ''
          const markerSuffix =
            titleOffset >= 0 ? line.text.slice(titleOffset + line.heading.title.length) : ''
          return (
            <Fragment key={key}>
              {lineIndex > 0 && '\n'}
              <Heading
