...
const MN4WritebackReview = dynamic(() => import("@/components/immersive-reading/MN4WritebackReview").then((m) => m.MN4WritebackReview), { ssr: false });

type SearchMode = "exact" | "fuzzy" | "description_fast" | "description_fine";
type ShelfView = "library" | "citations" | "focus-history" | "sync-review" | "vocabulary";
...
function ImmersiveReadingContent() {
 ...
 const [citations, setCitations] = useState<ReadingCitation[]>([]);
  ...
  const [mn4Writebacks, setMn4Writebacks] = useState<MN4WritebackItem[]>([]);
  const [mn4Loading, setMn4Loading] = useState(true);
  const [mn4Error, setMn4Error] = useState<string | null>(null);
 ...
 const refreshCitations = useCallback(async () => {
  ...
  const refreshMN4Writebacks = useCallback(async (showLoading = false) => {
    if (showLoading) setMn4Loading(true);
    setMn4Error(null);
    try {
      const data = await mn4WritebackApi.list();
      setMn4Writebacks(data.writebacks || []);
    } catch (cause) {
      setMn4Error(errorMessage(cause));
    } finally {
      if (showLoading) setMn4Loading(false);
    }
  }, []);

  useEffect(() => {
    void refreshMN4Writebacks(true);
  }, [refreshMN4Writebacks]);

  const updateMN4Writebacks = useCallback(
    async (writebackIds: string[], action: "approve" | "reject") => {
      if (!writebackIds.length) return;
      try {
        await (action === "approve"
          ? mn4WritebackApi.approve(writebackIds)
          : mn4WritebackApi.reject(writebackIds));
        await refreshMN4Writebacks();
      } catch (cause) {
        setMn4Error(errorMessage(cause));
      }
    },
    ...
    <div className="relative h-full overflow-y-auto bg-[var(--background)]">
      ...
      <main className="mx-auto w-full max-w-[1500px] px-8 py-7">
        <div className="mb-7 flex items-center gap-1 rounded-xl border border-[var(--border)] bg-[var(--card)] p-1.5 w-fit">
          {(["library", "citations", "vocabulary", "sync-review", "focus-history"] as const).map((view) => {
            ...
            >
             ...
             {view === "sync-review" && mn4Writebacks.some((item) => item.status === "pending_confirmation") && (
               <span className="rounded-full bg-current/10 px-1.5 text-[10px]">
                 {mn4Writebacks.filter((item) => item.status === "pending_confirmation").length}
               </span>
             )}
            </button>
          );})}
        ...
        {loading ? (
          ...
          />
        ) : shelfView === "sync-review" ? (
          <MN4WritebackReview
            items={mn4Writebacks}
            loading={mn4Loading}
            error={mn4Error}
            onApprove={(writebackIds) => updateMN4Writebacks(writebackIds, "approve")}
            onReject={(writebackIds) => updateMN4Writebacks(writebackIds, "reject")}
            onRetry={() => void refreshMN4Writebacks(true)}
          />
  ...
  const runTranslation = async () => {
    ...
    try {
      const result = await immersiveReadingApi.translate(
        ...
        {
          documentId,
          sourceObjectId: currentSection?.id || documentId,
        },
      );
...
const closeDictPopup = useCallback(() => {
 ...
 const translateDictWord = useCallback(async () => {
   ...
   setDictResult(null);
   try {
     const { translation } = await immersiveReadingApi.translate(
       dictPopup.word,
       "Chinese",
       undefined,
       { documentId, sourceObjectId: currentSection?.id || documentId },
     );
     setDictResult({
       word: dictPopup.word,
       phonetic: "",
 ...
 }, [dictPopup, documentId, currentSection?.id, t]);
